...
build_rootfs(){
		if [ ! -f $rootfs_image ]; then
			...
			echo "making f2fs disk image"
			dd if=/dev/zero of=flash0.img bs=1M count=500
			mkfs.f2fs flash0.img
...
run_qemu_debian(){
		qemu-system-x86_64 -m 2048\
			...
			-drive if=none,file=flash0.img,id=usb0 \
			-device virtio-blk-pci,drive=usb0 \
